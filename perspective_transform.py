...
def findVertices(points):
    ...
    # 上下左右四个点坐标,调整坐标调节画框工具
    # vertices = np.array([[top_point_x-50, top_point_y], [bottom_point_x+50, bottom_point_y], [left_point_x, left_point_y], [right_point_x, right_point_y]])
...
def tiltCorrection(vertices, rect):
    print (f'input_vec : {vertices} --- rect : {rect}')
    # 畸变情况1
    if rect[2] > -45:#（new_left_point对应0，0。其他位子有镜面对称性）
        new_right_point_x = vertices[0, 0] #top_point 的横坐标
        #相似三角。vertices[0, 0]- vertices[1, 0]（top-bottom横向距离）。(vertices[3, 0] - vertices[1, 0])（left_point -bottom横向距离），(vertices[1, 1] - vertices[3, 1])（left_point -bottom纵向距离
    ...
    # 畸变情况2
    elif rect[2] < -45:#（new_top_point对应0，0。其他位子有镜面对称性）
...
def main():
    path = '/Users/admin/data/patches/det_2018_09_13_000002_label_55_91_172_139.jpg'
    # 图像预处理
